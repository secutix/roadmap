...
/***/ function(module, exports, __webpack_require__) {
	...
	function linkRoom(room) {
		var roadmap = room.roadmap;
		var reference = room.reference;
		var threshold = room.threshold;
		...
		history.limitToLast(1).on('child_added', function (childSnapshot) {
			...
			(0, _jquery2['default'])('#user_' + event.visa).removeClass('user-active');
			setTimeout(function () {
				return (0, _jquery2['default'])('#user_' + event.visa).addClass('user-active');
			}, 10);
		});
		...
		history.on('child_added', function () {
			...
			if (historyLength !== -1) {
				if (historyLength++ > 10) {
					(0, _jquery2['default'])('#nav_stats').removeClass('hidden');
					historyLength = -1;
				} else {
					(0, _jquery2['default'])('#nav_stats').addClass('hidden');
				}
			}
		});
	}
	...
	base.onAuth(function (authData) {
		...
		if (authData) {
			...
			base.child('users/' + uid).on('value', function (data) {
				visa = data.val();
				roomName = (0, _jquery2['default'])('#room_input').val();
	
				// build room
				if (!hash) {
					if (roomName) {
						var ref = base.child('rooms').push();
						ref.set(roomName);
						hash = ref.toString().replace(/.*\/([^\/]+)$/g, '$1');
						window.location.hash = hash;
						(0, _jquery2['default'])('#new_room_overlay').removeClass('hidden');
						(0, _jquery2['default'])('#new_room_link').attr('href', window.location.href).text(window.location.href);
					} else {
						// logout
						base.unauth();
						return;
				...
				base.child('rooms/' + hash).once('value', function (roomData) {
					...
					currentRoom = new _room2['default'](base, 'room_' + hash, visa);
	...
	function showUserStats() {
		var userStats = currentRoom.userStats;
		...
		stats.forEach(function (stat) {
			(0, _jquery2['default'])('#stats_most_' + stat.id + ' .stats-card-visa').text(stat.most.visa);
			(0, _jquery2['default'])('#stats_less_' + stat.id + ' .stats-card-visa').text(stat.less.visa);
		});
	}
	...
	function showItemStats() {
		...
		var itemStats = currentRoom.itemStats;
		// get top 3
		...
		var _loop = function (index) {
			...
			var $container = (0, _jquery2['default'])('#stats_item_' + index);
	...
	(0, _jquery2['default'])(function () {
		// roadmap handlers
		(0, _jquery2['default'])('#nav_admin').on('click', function () {
			(0, _jquery2['default'])('#admin_block').toggleClass('hidden');
			(0, _jquery2['default'])('#main').toggleClass('admin');
			(0, _jquery2['default'])('#admin_roadmap_item').focus();
			return false;
		});
		(0, _jquery2['default'])('#admin_add').on('submit', function () {
			var $item = (0, _jquery2['default'])('#admin_roadmap_item');
			var value = $item.val();
			if (value) {
		...
		(0, _jquery2['default'])('#admin_threshold').on('submit', function () {
			var $item = (0, _jquery2['default'])('#admin_roadmap_threshold');
		...
		(0, _jquery2['default'])('#nav_reference').on('click', function () {
			currentRoom.reference.set(currentRoom.store);
			return false;
		});
	
		// upload / download
		(0, _jquery2['default'])('#roadmap_items_download').on('click', function () {
		...
		(0, _jquery2['default'])('body').on('dragover', function (event) {
		...
		function swap(direction) {
			return function () {
				var index = (0, _jquery2['default'])(this).parents('.roadmap-item').data('index');
				if (index === null) {
					return false;
				}
				// swap values
		...
		(0, _jquery2['default'])('#roadmap_items').on('click', '.roadmap-item-up', swap(-1)).on('click', '.roadmap-item-down', swap(1)).on('click', '.roadmap-item-delete', function () {
			var index = (0, _jquery2['default'])(this).parents('.roadmap-item').data('index');
			currentRoom.store.splice(index, 1);
			currentRoom.broadcast();
			return false;
		}).on('dragstart', '.roadmap-item', function (event) {
			if (!(0, _jquery2['default'])('#main').hasClass('admin')) {
				return false;
			}
			var originalEvent = event.originalEvent;
			...
			// of the next element
			setTimeout(function () {
				(0, _jquery2['default'])('#roadmap_items .roadmap-item:not(.roadmap-item-dragging)').addClass('roadmap-item-drop-target');
			}, 100);
		}).on('dragenter', '.roadmap-item', function () {
			var $this = (0, _jquery2['default'])(this);
			if ($this.hasClass('roadmap-item-drop-target')) {
				(0, _jquery2['default'])(this).addClass('roadmap-item-drop-target-enter');
			}
			...
			(0, _jquery2['default'])(this).removeClass('roadmap-item-dragging');
			(0, _jquery2['default'])('#roadmap_items .roadmap-item').removeClass('roadmap-item-drop-target').removeClass('roadmap-item-drop-target-enter');
		}).on('drop', '.roadmap-item', function (event) {
			var originalEvent = event.originalEvent;
			var targetIndex = (0, _jquery2['default'])(this).data('index');
		...
		// login handlers
		(0, _jquery2['default'])('#welcome form').on('submit', function (event) {
	
			event.preventDefault();
			// toggle display
			(0, _jquery2['default'])(this).addClass('hidden');
			(0, _jquery2['default'])('#welcome .progress').removeClass('hidden');
			var login = (0, _jquery2['default'])('#visa_input').val();
		...
		(0, _jquery2['default'])('#nav_logout').on('click', function () {
			base.unauth();
			return false;
		});
	
		(0, _jquery2['default'])('#nav_stats, #stats h2 .close').on('click', function () {
			(0, _jquery2['default'])('#stats, #main').toggleClass('hidden');
			showingStats = !showingStats;
			if (showingStats) {
				showUserStats();
				showItemStats();
				if ((0, _jquery2['default'])('#main').hasClass('admin')) {
					(0, _jquery2['default'])('#nav_admin').trigger('click');
				}
			}
			return false;
		});
		(0, _jquery2['default'])('#nav_toggle').on('click', function () {
			(0, _jquery2['default'])('nav .navbar-right').toggleClass('hidden-xs');
			return false;
		});
	
		// new room handlers
		(0, _jquery2['default'])('#new_room_continue').on('click', function () {
			(0, _jquery2['default'])('#new_room_overlay').addClass('hidden');
			return false;
		});
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	function createElement(text, index, total, changed, isReference, isCandidate) {
		...
		var $element = (0, _jquery2['default'])(markup.join(''));
	...
	function renderRoadmap(store, thresholdValue) {
		var $elementContainer = (0, _jquery2['default'])('#roadmap_items');
		var newStoreView = [];
		var nbItems = store.length;
		$elementContainer.html('');
		if (nbItems) {
			(0, _jquery2['default'])('#roadmap_items_tips').addClass('hidden');
			...
			(0, _jquery2['default'])('#roadmap_items_tips').removeClass('hidden');
		}
	...
	function renderReference(storeReference, thresholdValue) {
		var $elementContainer = (0, _jquery2['default'])('#reference_items');
		$elementContainer.html('');
	...
	function renderActionNotification(index, event) {
		var $item = (0, _jquery2['default'])('#roadmap_item_' + index + ' > div');
		$item.removeClass('roadmap-item-notif-up, roadmap-item-notif-down');
		(0, _jquery2['default'])('.roadmap-author', $item).text(event.visa).removeClass('roadmap-author-new');
		setTimeout(function () {
			$item.addClass('roadmap-item-notif-' + (event.direction < 0 ? 'up' : 'down'));
			(0, _jquery2['default'])('.roadmap-author', $item).addClass('roadmap-author-new');
		}, 10);
	}
	...
	function loginView(hash) {
		(0, _jquery2['default'])('#welcome, #welcome form').removeClass('hidden');
		(0, _jquery2['default'])('#welcome .progress').addClass('hidden');
		(0, _jquery2['default'])('#visa_input').focus();
		(0, _jquery2['default'])('#main, #stats, nav, footer').addClass('hidden');
		(0, _jquery2['default'])('#admin_block').addClass('hidden');
		(0, _jquery2['default'])('#room_input').toggleClass('hidden', !!hash);
	}
	
	function roadmapView(visa, roomName) {
		(0, _jquery2['default'])('#main, nav, footer').removeClass('hidden');
		(0, _jquery2['default'])('#welcome').addClass('hidden');
		(0, _jquery2['default'])('#visa').text(visa);
		(0, _jquery2['default'])('#room_name').text(roomName);
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Room = (function () {
		function Room(base, id, visa) {
			var _this = this;
	
			_classCallCheck(this, Room);
	
			var room = base.child(id);
			...
			this.history.on('child_added', function (childSnapshot) {
				var event = childSnapshot.val();
				var time = event.time;
				var now = Date.now();
				...
				_this.savedStats.push(event);
	
				var eventVisa = event.visa;
				var direction = event.direction;
				var text = event.value;
				var user = helper.getOrSet(_this.userStats, eventVisa, {
					up: 0,
					down: 0,
					total: 0,
					toRef: 0,
				...
				var item = helper.getOrSet(_this.itemStats, text, {
					up: 0,
					total: 0,
					upUsers: {},
					downUsers: {},
		...
		_createClass(Room, [{
			...
			key: 'swap',
			value: function swap(index, direction, visa) {
